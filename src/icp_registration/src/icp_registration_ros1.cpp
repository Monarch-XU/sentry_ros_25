#include "icp_registration/icp_registration_ros1.h"


namespace icp{

IcpNode::IcpNode(ros::NodeHandle& nh) : nh_(nh){
    ...
    nh_.param<double>("rough_leaf_size", rough_leaf_size, 1);
    nh_.param<double>("refine_leaf_size", refine_leaf_size, 1);
    ...
    // 读取PCD文件路径参数
    nh_.param<std::string>("pcd_path", pcd_path_, "/home/zcf/Downloads/RMUL.pcd");
    ...
    nh_.param<std::string>("range_odom_frame_id", range_odom_frame_id_, "range_odom");
    nh_.param<std::string>("laser_frame_id", laser_frame_id_, "laser");
    nh_.param<double>("thresh", thresh_,  0.15);
    nh_.param<double>("xy_offset", xy_offset_,  0.2);
    nh_.param<double>("yaw_offset", yaw_offset_,  30.0);
    yaw_offset_ *= M_PI / 180.0;

    std::vector<double> initial_pose_vec; 
    nh_.param<std::vector<double>>("initial_pose", initial_pose_vec,  {0, 0, 0, 0, 0, 0});
    ...
    nh_.param<std::string>("pointcloud_topic", pointcloud_topic,  "/livox/lidar/pointcloud");


    ROS_INFO("pointcloud_topic: %s",pointcloud_topic.c_str());

    //// TODO:这行不是很明白：boost::bind(&IcpRegistration::pointcloudCallback, this, _1)
    
    // pointcloud_sub_ = nh_.subscribe(pointcloud_topic,10,boost::bind(&IcpNode::pointcloudCallback, this, std::placeholders::_1));
    // pointcloud_sub_ = nh_.subscribe(pointcloud_topic, 10, [this](const sensor_msgs::PointCloud2ConstPtr& msg) {
    //     this->pointcloudCallback(msg);
    // });
    
    initial_pose_sub_ = nh_.subscribe("/initialpose",10,boost::bind(&IcpNode::initialPoseCallback, this, std::placeholders::_1));
...
void IcpNode::livox_pcl_cbk(const std::string &msg) {
    ROS_INFO("being used\n");
}

void IcpNode::pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr &msg) {
...
void IcpNode::quaternionToEigenMatrix(const tf::Quaternion& q, Eigen::Matrix3d& rotation_matrix) {
    // 设置旋转矩阵的元素
