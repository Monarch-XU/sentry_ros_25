...
// 如果同时有初始坐标和点云，则进入点云配准循环；如果没有同时接收到，则循环发布
// 0偏移量的map到odom的tf变换关系

// 有点云的条件：点云大小不为零；接受标志符号为true；
// 有坐标接收的条件：接受标志符号为true；
...
int main(int argc, char *argv[]) {
    ...
    nh.param<double>("rough_leaf_size", rough_leaf_size_, 0.4);
    nh.param<double>("refine_leaf_size", refine_leaf_size_, 0.1);

    // 初始化坐标变换器
    map_to_odom_transform.setOrigin(tf::Vector3(0.0, 0.0, 0.0));
    map_to_odom_transform.setRotation(tf::Quaternion(0.0, 0.0, 0.0, 0.0));

    // ROS_INFO("refine_leaf_size: %lf\n",refine_leaf_size_);

    //  // 设置粗糙和精细体素滤波器叶大小。
    // voxel_rough_filter_.setLeafSize(rough_leaf_size_, rough_leaf_size_, rough_leaf_size_);
    // voxel_refine_filter_.setLeafSize(refine_leaf_size_, refine_leaf_size_, refine_leaf_size_);
...
void pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr msg) {
    ...
    if(cloud_in_->points.size()<=0){
        ...
        map_to_odom_transform.setOrigin(tf::Vector3(2.0, 2.0, 0.0));
        map_to_odom_transform.setRotation(tf::Quaternion(0.0, 0.0, 0.0, 0.0));
        ROS_INFO("CHANGE POSITION!\n");
    }
