...
/////////////////////////////////////////////前哨站//////////////////////////////////////////////
    ...
    if( (Tracker.tracking_id == 8 || Tracker.tracking_id == 7) && Outpost_Mode == 1 ){
        PO.Outpost_Track(Tracker.enemy_armor,std::chrono::high_resolution_clock::now());

        double select_angle_left = -10;
        ...
        if(spin_angle < select_angle_left && spin_angle > select_angle_right){
            ...
            Tracker.Solve_pitch = rpy[1];
            Tracker.Solve_yaw = rpy[2];
            // ROS_INFO("----COUNTER_CLOCKWISE YES----");
        }
   ...
   // 开火指令判断
    ...
    if(Fire_X_OK && Fire_Y_OK){
        ...
        if(Fire_OK){
            Fire = Fire_ON;
            Fire_mode = Three_shoot;
        }else if(Tracker.tracking_id != 8 && Tracker.tracking_id != 7){
            Fire = Fire_ON;
            Fire_mode = Continuous_shoot;
        }
...
// 获取模式
void Vision_mode(const std_msgs::Int8::ConstPtr &Vision_Mode){
    Outpost_Mode = Vision_Mode->data;
}
...
int main(int argc, char *argv[]){
    ...
    PTZ_perception_pub = nh.advertise<robot_msgs::PTZ_perception>("/PTZ_perception_L",1);
    reset_time = ros::Time::now();

    ros::Subscriber Track_reset_sub = nh.subscribe<robot_msgs::Track_reset>("/PTZ_L/Track_Reset",1,Track_Reset_CMD);   
    ros::Subscriber mode_sub = nh.subscribe<std_msgs::Int8>("outpost_attack_flag",1,Vision_mode);  
