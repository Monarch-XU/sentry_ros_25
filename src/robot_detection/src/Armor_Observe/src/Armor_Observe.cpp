...
//        pre_cir = AS.imu2pixel(pre_pos);
        ...
        if(spin_angle*(180.0f/CV_PI) > 0){
            O_Armor_angle = spin_angle*(180.0f/CV_PI) - 180.0;
        }else{
            O_Armor_angle = spin_angle*(180.0f/CV_PI) + 180.0;
        }
        ...
        double lock_angle = 15;
        cv::Point2f left_target_Armor;                    // 左侧装甲板点位
        cv::Point2f right_target_Armor;                   // 右侧装甲板点位
        left_target[0] = center_temp[0] - sin(lock_angle*(CV_PI/180.0f))*RL_length;
        left_target[1] = center_temp[1] - cos(lock_angle*(CV_PI/180.0f))*RL_length;
        left_target[2] = center_temp[2]+(RL_height/2);
        right_target[0] = center_temp[0] + sin(lock_angle*(CV_PI/180.0f))*RL_length;
        right_target[1] = center_temp[1] - cos(lock_angle*(CV_PI/180.0f))*RL_length;
